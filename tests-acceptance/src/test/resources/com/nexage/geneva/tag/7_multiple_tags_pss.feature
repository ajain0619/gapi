Feature: Seller - Sites - Tags: create multiple tags as PSS Admin

  Background: log in as nexage admin
    Given the user "user" has logged in with role "AdminNexage"
  ...
  Scenario: update global config adSource limit to 30 - to accommodate more tags
    When the user updates limit of property "seller.tags.per.position.limit" global config to "30"
