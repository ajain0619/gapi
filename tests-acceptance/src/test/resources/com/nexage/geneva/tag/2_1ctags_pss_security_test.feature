...
Feature: Seller - Sites - Tags: access/permission tests
  ...
  Scenario Outline: All publisher/seller roles can GET tags
    ...
    And the user creates a duplicate user in db from the json file "jsons/genevacrud/tag/permission/create/<usr_file_payload>_payload.json"
    ...
    Examples:
      | usr_file_payload | username           | role          |
      | SellerUser       | 105_SELLER_USER    | UserSeller    |
      | SellerAdmin      | 105_SELLER_ADMIN   | AdminSeller   |
      | SellerManager    | 105_SELLER_MANAGER | ManagerSeller |
  ...
  Scenario Outline: Create a user with User role and check that he cannot PUT tags
    ...
    Examples:
      | usr_file_payload | username        | role        | type         | tagName                  | tag_file_payload                              | file_ER                                  |
      | SellerUser       | 105_SELLER_USER | UserSeller  | non-exchange | Nexage-Adserver - test8c | UpdateMediationAdSourceRequiredFields_payload | UpdateMediationAdSourceRequiredFields_ER |
      | SellerUser       | 105_SELLER_USER | UserSeller  | exchange     | Nexage Exchange - test8x | UpdateMillennialMediaRequiredFields_payload   | UpdateMillennialMediaRequiredFields_ER   |
  ...
  Scenario Outline: Create a user with User role and check that he cannot POST tags
    ...
    Examples:
      | usr_file_payload | username        | role       | type         | tag_file_payload                                         | file_ER                                             |
      | SellerUser       | 105_SELLER_USER | UserSeller | exchange     | NewMillennialMediaExchangeAdSourceRequiredFields_payload | NewMillennialMediaExchangeAdSourceRequiredFields_ER |
      | SellerUser       | 105_SELLER_USER | UserSeller | non-exchange | NewMediationAdSourceRequiredFields_payload               | NewMediationAdSourceRequiredFields_ER               |
