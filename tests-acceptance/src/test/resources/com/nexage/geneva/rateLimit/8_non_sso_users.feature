@onemobilelogin
Feature: create multiple non sso users tags, update global users limit, update seller_attribute users limit
  ...
  Scenario Outline: create new users
    ...
    # 4 is the user limit - all users should be created
    Given the user "pssSellerAdmin" has logged in
    When the non sso user creates a user from the json file "jsons/genevacrud/rateLimit/payload/user/User_payload.json" with name in "<names>" and email in "<email>" and role in "<roles>" and set status in "<status>"
    Then request passed successfully
    And non sso users "<names>" can be searched in the database
  ...
  Scenario: additional user to exceed the limit
    # this should not be seen in the database
    Given the user "pssSellerAdmin" has logged in
    When the non sso user creates a user from the json file "jsons/genevacrud/rateLimit/payload/user/User_exceeded_payload.json" with name in "rateLimitSellermanager4" and email in "01jdffgd@rea.ru" and role in "ROLE_USER" and set status in "true"
    Then "user creation" failed with "400" response code and error message "Limit reached for this seller"
    And non sso users "rateLimitSellermanager4" cannot be searched in the database

  Scenario: update Seller company with user limit equals to 4
    Given the user "crudnexageadmin" has logged in
  ...
  Scenario: update Seller company from pss and verify the site limit equals to 4 didnt change
    Given the user "crudnexageadmin" has logged in
    And the user selects the "Seller" company "CRUDPositionTest"
    And the user updates a publisher from the json file "jsons/genevacrud/rateLimit/payload/user/UpdateSeller10201FromPssUserLimit_payload.json"
    Then request passed successfully
    And the user limit field value for seller CRUDPositionTest is preserved in the db
  ...
  Scenario: create new user
    ...
    When the non sso user creates a user from the json file "jsons/genevacrud/rateLimit/payload/user/User_10201_payload.json" with name in "rateLimitSellerUser3" and email in "01dfgfsd@ew.ri" and role in "ROLE_USER" and set status in "true"
    Then request passed successfully
    And non sso users "rateLimitSellerUser3" can be searched in the database
  ...
  Scenario: recreate additional user to exceed the limit
    ...
    Given the user "pssSellerAdmin" has logged in
    When the non sso user creates a user from the json file "jsons/genevacrud/rateLimit/payload/user/User_exceeded_payload.json" with name in "rateLimitSellermanager6" and email in "01jdffgd@rea.ru" and role in "ROLE_USER" and set status in "true"
    Then request passed successfully
    And non sso users "rateLimitSellermanager6" can be searched in the database
  ...
  Scenario: create new user
    ...
    When the non sso user creates a user from the json file "jsons/genevacrud/rateLimit/payload/user/User_10201_payload.json" with name in "rateLimitSellerUser3" and email in "01dfgfsd@ew.ri" and role in "ROLE_USER" and set status in "true"
    Then "user creation" failed with "400" response code and error message "Limit reached for this seller"
    And non sso users "rateLimitSellerUser3" cannot be searched in the database
  ...
  Scenario Outline: create new users
    ...
    Examples:
      | user              | names             | email                   | roles        | status |
      | crudnexageadmin   | NexageAdmin1105   | 01nexageadmin1@ex.com   | ROLE_ADMIN   | true   |
      | crudnexageadmin   | NexageAdmin2      | 01sdfhd@q.tt            | ROLE_ADMIN   | true   |
      | crudnexagemanager | NexageManager1105 | 01nexagemanager1@ex.com | ROLE_MANAGER | true   |
      | crudnexagemanager | NexageManager2    | 01dfhs@rqe.fg           | ROLE_MANAGER | true   |

  Scenario Outline: create new users for a different seller
    # 1 existing user for publisher 10201
    # 1 is the user limit - all 4 users should be created
    Given the user "<user>" has logged in
    When the non sso user creates a user from the json file "jsons/genevacrud/rateLimit/payload/user/User_10201_payload.json" with name in "<names>" and email in "<email>" and role in "<roles>" and set status in "<status>"
    Then request passed successfully
    And non sso users "<names>" can be searched in the database

    Examples:
      | user              | names               | email                | roles        | status |
      | crudnexageadmin   | NexageAdmin110201   | 01nexageuser1@ex.com | ROLE_ADMIN   | true   |
      | crudnexageadmin   | NexageAdmin210201   | 01sdf@qw.yy          | ROLE_ADMIN   | true   |
      | crudnexagemanager | NexageManager110201 | 01hdfsfsa@qw.ru      | ROLE_MANAGER | true   |
      | crudnexagemanager | NexageManager210201 | 01jgdfjdf@qew.ru     | ROLE_MANAGER | true   |
