...
public class AuthorizationCodeAccessTokenProvider extends OAuth2AccessTokenSupport
  ...
  public AuthorizationCodeAccessTokenProvider(final String realm) {
    this.realm = realm;
  }
  ...
  private MultiValueMap<String, String> getParametersForTokenRequest(
    ...
    // Extracting the redirect URI from a saved request should ignore the current URI, so it's not
    // simply a call to
  ...
  private UserRedirectRequiredException getRedirectForAuthorization(
      AuthorizationCodeResourceDetails resource, AccessTokenRequest request) {
    ...
    UserRedirectRequiredException redirectException =
        new UserRedirectRequiredException(resource.getUserAuthorizationUri(), requestParameters);
  ...
  protected UserApprovalRequiredException getUserApprovalSignal(
      AuthorizationCodeResourceDetails resource, AccessTokenRequest request) {
    String message =
        String.format(
            "Do you approve the client '%s' to access your resources with scope=%s",
            resource.getClientId(), resource.getScope());
    return new UserApprovalRequiredException(
        resource.getUserAuthorizationUri(),
        Collections.singletonMap(OAuth2Utils.USER_OAUTH_APPROVAL, message),
