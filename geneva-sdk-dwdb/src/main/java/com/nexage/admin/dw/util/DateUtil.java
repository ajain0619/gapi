...
import com.nexage.admin.dw.util.ReportDefEnums.DateRange;
...
public final class DateUtil {
   ...
   * @return Date
   */
  private static Date roundTimeToSeconds(Date date) {
    if (date == null) {
  ...
  public static Map<String, String> getAllDatesBetween(String start, String stop)
    ...
    while (eCal.after(sCal)) {
      dates.put(
          DATE_FORMAT.format(ZonedDateTime.ofInstant(sCal.toInstant(), ZoneId.systemDefault())),
          format((sCal.getTime())));
  ...
  public static Map<String, String> getAllWeeksBetween(String start, String stop)
    ...
    while (eCal.after(sCal)) {
      ...
      String week =
          new StringBuilder(String.valueOf(year))
              .append("-")
              .append(String.valueOf(sCal.get(Calendar.WEEK_OF_YEAR) - 1))
              .toString();
      dates.put(week, format(sCal.getTime()));
  ...
  public static Map<String, String> getAllMonthsBetween(String start, String stop)
    ...
    while (eCal.after(sCal)) {
      dates.put(
          MONTH_FORMAT.format(ZonedDateTime.ofInstant(sCal.toInstant(), ZoneId.systemDefault())),
          format((sCal.getTime())));
  ...
  public static Map<String, String> getAll24HoursForDate(String date) {
    Map<String, String> dates = new HashMap<>();
    for (int i = 0; i < 24; i++) {
  ...
  public static Date parseFromMonthFormat(String date) {
  ...
  public static Date parseFromDateFormat(String date) {
    return Date.from(
        LocalDate.parse(date).atStartOfDay().atZone(ZoneId.systemDefault()).toInstant());
  }

  public static String fomatDateToGivenTZ(Date date, final TimeZone tz) {
