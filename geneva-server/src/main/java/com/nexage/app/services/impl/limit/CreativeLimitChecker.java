 ...
 * {@link CreativeLimitInterceptor} that encapsulates the real checker. Spring AOP does not allow
...
@Log4j2
@Service
public class CreativeLimitChecker {

  /**
   * Check User Rate Limits for creatives in campaigns to see if the limit has been reached.
   *
   * <p>{@link CreativeLimitInterceptor#canCreateCreativesInCampaign(JoinPoint, Long, Long)}
   *
   * @param sellerPid sellerPid
   * @param campaignPid campaignPid
   */
  public void checkLimitsCreativeInCampaign(Long sellerPid, Long campaignPid) {
    log.info("checkLimitsCreativeInCampaign, sellerPid={}, campaignPid={}", sellerPid, campaignPid);
  }
}
